 ...
 */
public class KadActionsBuilder implements ActionsBuilder<KadAction, SMSPeer, StringResource,
        BinarySet> {
    ...
    private static final String ILLEGAL_MAX_ID_MSG = "Can't initialize Builder, the defined max " +
            "ID isn't compatible with class KadActions";
    ...
    /**
     * Constructor for this builder, defines a new max limit for IDs, min limit remains the one
     * defined in KadAction
     *
     * @param maxActionID max value for action IDs that this builder will accept, must be a valid
     *                    KadAction ID
     */
     ...
     * @return if the given ID is valid: an action built to execute the ping command to the peer,
     * otherwise returns an INVALID_ACTION
     */
    @Override
    ...
    /**
     * @param request the ping request received from a peer (that will be the receiver for the
     *                response)
     * @return a response action built to respond to the received ping command from the peer, or
     * an invalid KAD action if the given action wasn't coherent with this building method
     * (actionType)
     */
    @Override
     ...
     * @param actionID the unique ID of the action
     * @return if the given ID is valid: an action built to execute the invite command to the
     * peer, otherwise returns an INVALID_ACTION
     */
    @Override
     ...
     * @return a response action built to respond to the received invite, or an invalid KAD
     * action if the given action wasn't coherent with this building method (actionType)
     */
    @Override
     ...
     * @param target   the node to be stored
     * @return if the given ID is valid: an action built to execute the store (node) command to
     * the peer, otherwise returns an INVALID_ACTION
     */
    @Override
    public KadAction buildStore(int actionID, SMSPeer peer, BinarySet target) {
        if (isValidID(actionID))
            return new KadAction(
                    peer,
     ...
     * @return if the given ID is valid: an action built to execute the store (resource) command
     * to the peer, otherwise returns an INVALID_ACTION
     */
    @Override
     ...
     * @return an array of responses (one for each peer in peers) for the received request, or an
     * invalid KAD action if the given action wasn't coherent with this building method (actionType)
     */
    @Override
     ...
     * @return a response action built to respond to the received store (resource), or an invalid
     * KAD action if the given action wasn't coherent with this building method (actionType)
     */
    @Override
     ...
     * @param target   the node of which find its address (on the used network)
     * @return if the given ID is valid: an action built to execute the find node command to the
     * peer, otherwise returns an INVALID_ACTION
     */
    @Override
    public KadAction buildFindNode(int actionID, SMSPeer peer, BinarySet target) {
        return new KadAction(
                peer,
     ...
     * @param peers   the peers that are closest to the searched node (or the searched node's
     *                corresponding peer)
     * @return an array of responses (one for each peer in peers) for the received request, or an
     * invalid KAD action if the given action wasn't coherent with this building method (actionType)
     */
     ...
     * @param actionID the unique ID of the action
     * @param peer     the peer that will receive the built action command
     * @param target   the node representing the searched resource
     * @return if the given ID is valid: an action built to execute the find value command to the
     * peer, otherwise returns an INVALID_ACTION
     */
    @Override
    public KadAction buildFindValue(int actionID, SMSPeer peer, BinarySet target) {
        if (isValidID(actionID))
            return new KadAction(
                    peer,
     ...
     * @param resource if found the searched resource (by the requesting peer), otherwise a
     *                 resource containing a fixed token (that indicated that the resource hasn't
     *                 been found)
     * @return a response action built to respond to the received find value command (of a
     * resource), or an invalid KAD action if the given action wasn't coherent with this building
     * method (actionType)
     */
    @Override
